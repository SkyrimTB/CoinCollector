...
GameRunner<T>::GameRunner()
    : isRunning(true), currentLevel(1), playerHP(4), maxHP(4), score(0),
      coinTarget(50), fireCount(2), itemRounds(5), stepCount(0) {
  displayInstructions();
  initGame();
}

// Destructor
template <typename T> GameRunner<T>::~GameRunner() { isRunning = false; }
...
GameRunner<T>::GameRunner(GameRunner &&other) noexcept
    : isRunning(std::exchange(other.isRunning, false)),
      ...
      playerY(std::exchange(other.playerY, 0)), fires(std::move(other.fires)),
      coin(std::move(other.coin)), item(std::move(other.item)),
...
// Run the game
template <typename T> void GameRunner<T>::run() {
  startGameLoop();
...
// Initialize the game
template <typename T> void GameRunner<T>::initGame() {
...
template <typename T> void GameRunner<T>::placeCoin() {
  int x, y;
  do {
    x = std::rand() % width;
    y = std::rand() % height;
...
// Place the healing item
template <typename T> void GameRunner<T>::placeItem() {
  auto validPositions =
      gameBoard | std::views::filter([](const auto &row) {
        return std::ranges::any_of(row, [](char cell) { return cell == ' '; });
      });

  auto [pos] = *std::ranges::begin(validPositions);
  int y = pos[0];
  int x = pos[1];
  gameBoard[y][x] = 'H';
...
// Game loop
template <typename T> void GameRunner<T>::gameLoop() {
  int roundsInLevel = 0;
  while (isRunning) {
    renderGame();
    ...
    if (score >= coinTarget) {
      currentLevel++;
      if (currentLevel <= 3) {
        std::cout << std::format("Congratulations! You collected enough coins "
                                 "to advance to level {}.\n",
                                 currentLevel);
        coinTarget += 50;
        roundsInLevel = 0;
...
// Reset the game
template <typename T> void GameRunner<T>::resetGame() {
  currentLevel = 1;
  playerHP = maxHP;
  score = 0;
  coinTarget = 50;
...
// Start the game loop
template <typename T> void GameRunner<T>::startGameLoop() {
  gameThread = std::jthread(&GameRunner::gameLoop, this);
}

// Render the game board
template <typename T> void GameRunner<T>::renderGame() {
...
// Process user input
template <typename T> void GameRunner<T>::processInput() {
  while (isRunning) {
...
template <typename T> void GameRunner<T>::movePlayer(char input) {
  int dx = 0, dy = 0;
  switch (input) {
  case 'w':
    dy = -1;
  ...
  if (newX >= 0 && newX < width && newY >= 0 && newY < height) {
    if (gameBoard[newY][newX] == ' ' || gameBoard[newY][newX] == 'C' ||
        (newX == item.first && newY == item.second && itemPresent)) {
...
// Update the fires on the game board
template <typename T> void GameRunner<T>::updateFires(int roundsInLevel) {
...
// Update the healing item on the game board
template <typename T> void GameRunner<T>::updateItem() {
  currentItemRound++;
...
template <typename T> void GameRunner<T>::checkCollisions() {
...
template <typename T> void GameRunner<T>::displayInstructions() {
  std::cout << "Welcome to the Coin Collector Game!\n";
  std::cout
      << "Objective: Collect coins to advance levels while avoiding fires.\n";
  std::cout << "Controls:\n";
  std::cout << "  - Move Up: W\n";
